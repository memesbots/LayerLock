    const EN_TEXT = Object.freeze({
      ...
      "Мастер-ключ слишком короткий: используйте не менее 16 символов.": "The master key is too short: use at least 16 characters.",
      "Пароль слоя слишком короткий: используйте не менее 12 символов.": "The layer password is too short: use at least 12 characters.",
      "Этот пароль слишком распространен или легко угадывается.": "This password is too common or easy to guess.",
      ...
      "Мастер-ключ должен содержать не менее 16 символов, пароль слоя — не менее 12. Одинаковые, распространенные и предсказуемые пароли отклоняются. Секретные поля и открытый результат очищаются после 15 минут бездействия.": "The master key must contain at least 16 characters and each layer password at least 12. Duplicate, common, and predictable passwords are rejected. Secret fields and decrypted results are cleared after 15 minutes of inactivity.",
    ...
    const EN_PATTERNS = Object.freeze([
      ...
      [/^Слой (\d+): Пароль слоя слишком короткий: используйте не менее 12 символов\.$/, (_, n) => `Layer ${n}: the layer password is too short; use at least 12 characters.`],
      [/^Слой (\d+): Этот пароль слишком распространен или легко угадывается\.$/, (_, n) => `Layer ${n}: this password is too common or easy to guess.`],
